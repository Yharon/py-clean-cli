...
class HelloCommand(CommandArgsModel):
    ...
    def exec(self) -> None:
        ...
        if self.verbose:
            LOGGER.info(f"Generating greeting with parameters: name='{self.name}', greeting='{self.greeting}', uppercase={self.uppercase}")
...
class UserCommand(CommandArgsModel):
    ...
    def _handle_create_operation(self) -> None:
        ...
        if self.dry_run:
            ...
            LOGGER.info(f"Creating new user: {self.username} ({self.email}) with role: {self.role}")
